...
fs.readdirSync(vocabFolder).filter(fn => fn.endsWith('.xml')).forEach(xmlfile => {
    ...
    // var json = csdl.xml2json(xml, false);
    ...
    //TODO:
    // - create markdown in /scn from json without line numbers
    // - call github REST API (not via curl) to get SCN markup
    // - use .replace instead of sed to tweak it
    // - fs.writeFileSync(...)
});