...
[ServiceDefault](Common.xml#L1684)|EntityType?|<a name="ServiceDefault"></a>Entity-valued term that signals a service-specific default expansion of the response<br>A client that requests `$expand=@Common.ServiceDefault` allows the service to determine the expansion of navigation properties in the response. (Unlike the default expansion in the absence of `$expand`, this need not include all entities or properties from the request.) This term is never applied and services omit the null value for this annotation in the response.
