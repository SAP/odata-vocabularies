...
[StreamOperation](Common.xml#L1650) *([Experimental](Common.md#Experimental))*|PropertyPath|<a name="StreamOperation"></a>The annotated bound action is always combined with an update of a stream property<br><p>The value of the annotation is a property path to a stream property of the return type of the action. To invoke the operation the client makes two requests in one LUW:</p> <ul> <li>a POST request that invokes the action followed by</li> <li>a PUT or DELETE request on the stream property of the action result (not in a $batch).</li> </ul> <p>Both requests become effective only if the second request succeeds.</p> 
