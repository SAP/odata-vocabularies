...
[SourceFacets](Common.xml#L1066)|\[AnnotationPath\]|Each `Edm.AnnotationPath` must describe a UI facet, i.e., must be valid for [`UI.ReferenceFacet/Target`](UI.md#ReferenceFacet). The side effect targets may be affected by changes to the values of one or more of the structural properties that make up the referenced facets.
