...
[ChangeViaNavigationPath](Common.xml#L1526) *([Experimental](Common.md#Experimental))*|[Tag](https://github.com/oasis-tcs/odata-vocabularies/blob/main/vocabularies/Org.OData.Core.V1.md#Tag)|<a name="ChangeViaNavigationPath"></a>Service prefers change requests to use a resource path with navigation properties<br>Use this tag on services that do not restrict update and delete operations to certain resource paths via `Capabilities` annotations, but that prefer update and delete operations on a resource path that contains the navigation properties expressing the object composition.
