...
[DrillStateProperty](./Hierarchy.xml#L83:~:text=<ComplexType%20Name="-,RecursiveHierarchyType,-")|PropertyPath?|String property holding the drill state of a node<br>Possible drill states are: <br>- `expanded` if a node has children in the result set <br>- `collapsed` if a node has children in the hierarchical output set but not in the result set <br>- `leaf` if a node has no children in the hierarchical output set
