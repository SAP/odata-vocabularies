...
Parameter|Type|Description
:--------|:---|:----------
**[Node](Hierarchy.xml#L418)**|EntityType|**Binding parameter:** The node to be copied
[&rarr;](Hierarchy.xml#L421)|EntityType|The copied node
...
Parameter|Type|Description
:--------|:---|:----------
**[InputSet](Hierarchy.xml#L310)**|\[EntityType\]|**Binding parameter**
[HierarchyNodes](Hierarchy.xml#L311)|\[EntityType\]|A collection, given through a `$root` expression
[HierarchyQualifier](Hierarchy.xml#L314)|[HierarchyQualifier](https://github.com/oasis-tcs/odata-vocabularies/blob/main/vocabularies/Org.OData.Aggregation.V1.md#HierarchyQualifier)|
[NodeProperty](Hierarchy.xml#L315)|String|Property path to the node identifier, evaluated relative to the binding parameter
*[Levels](Hierarchy.xml#L318)*|Int64|*Optional parameter:* The number n of levels to be output, absent means all levels
*[Show](Hierarchy.xml#L325)*|\[String\]|*Optional parameter:* Identifiers of nodes to be shown
*[ExpandLevels](Hierarchy.xml#L331)*|\[[TopLevelsExpandType](#TopLevelsExpandType)\]|*Optional parameter:* Nodes to be expanded
[&rarr;](Hierarchy.xml#L337)|\[EntityType\]|
...
Property|Type|Description
:-------|:---|:----------
[ExternalKey](Hierarchy.xml#L126) *([Experimental](Common.md#Experimental))*|String?|Human-readable key value for a node<br>If a `NodeType` exists, the external key is unique only in combination with it. Or the external key can coincide with the [`Aggregation.RecursiveHierarchy/NodeProperty`](https://oasis-tcs.github.io/odata-vocabularies/vocabularies/Org.OData.Aggregation.V1.html#RecursiveHierarchyType).
[NodeType](Hierarchy.xml#L134) *([Experimental](Common.md#Experimental))*|String?|Type of a node<br><p>In a recursive hierarchy with mixed types, nodes can</p> <ul> <li>have a type-specific (navigation) property whose name is the node type or</li> <li>be represented by entities of different subtypes of a common entity type that is</li> </ul> <p>annotated with the <code>RecursiveHierarchy</code> annotation. The qualified name of the subtype is the node type.</p> 
[LimitedDescendantCount](Hierarchy.xml#L145)|Int64?|Number of descendants a node has in the limited hierarchy
[DrillState](Hierarchy.xml#L148)|String?|Drill state of an occurrence of a node<br>Allowed Values:<dl><dt>[expanded](Hierarchy.xml#L152)<dd>The node has children in the limited hierarchy<dt>[collapsed](Hierarchy.xml#L156)<dd>The node has children in the unlimited hierarchy but not in the limited hierarchy<dt>[leaf](Hierarchy.xml#L160)<dd>The node has no children in the unlimited hierarchy</dl>
[DistanceFromRoot](Hierarchy.xml#L167)|Int64?|Number of ancestors an occurrence of a node has in the limited hierarchy<br>This equals the number of ancestors in the sub-hierarchy, if the request involves a sub-hierarchy.
[ChildCount](Hierarchy.xml#L173) *([Experimental](Common.md#Experimental))*|Int64?|Number of children a node has in the unlimited hierarchy
[DescendantCount](Hierarchy.xml#L177) *([Experimental](Common.md#Experimental))*|Int64?|Number of descendants a node has in the unlimited hierarchy
[LimitedRank](Hierarchy.xml#L181)|Int64?|Rank of a node in the limited hierarchy in preorder or postorder<br>The rank of a node is the index of the node in the sequence of nodes created from a preorder or postorder traversal of the limited hierarchy. The first node in the traversal has rank 0.
[SiblingRank](Hierarchy.xml#L188) *([Experimental](Common.md#Experimental))*|Int64?|Sibling rank of a node<br>The sibling rank of a node is the index of the node in the sequence of all nodes in the unlimited hierarchy with the same parent. The first sibling has rank 0.
[Matched](Hierarchy.xml#L196) *([Experimental](Common.md#Experimental))*|Boolean?|Flag indicating [matching](#MatchCount) nodes
[MatchedDescendantCount](Hierarchy.xml#L200) *([Experimental](Common.md#Experimental))*|Int64?|Number of [matching](#MatchCount) descendants a node has in the unlimited hierarchy

<a name="RecursiveHierarchyActionsType"></a>
## [RecursiveHierarchyActionsType](Hierarchy.xml#L218)
...
Property|Type|Description
:-------|:---|:----------
[ChangeNextSiblingAction](Hierarchy.xml#L248)|[QualifiedName?](Common.md#QualifiedName)|Action that moves a node among its siblings, following [this template](#Template_ChangeNextSiblingAction)
[ChangeSiblingForRootsSupported](Hierarchy.xml#L258)|Boolean|Whether the sibling of a root can be changed
[CopyAction](Hierarchy.xml#L261)|[QualifiedName?](Common.md#QualifiedName)|Action that copies a node and its descendants, following [this template](#Template_CopyAction)

<a name="TopLevelsExpandType"></a>
## [TopLevelsExpandType](Hierarchy.xml#L339)
Information about nodes to be expanded

Property|Type|Description
:-------|:---|:----------
[NodeID](Hierarchy.xml#L341)|String|Identifier of a node to be expanded
[Levels](Hierarchy.xml#L344)|Int64?|Number of levels to be expanded, null means all levels, 0 means collapsed
