...
[MatchedIndicatorsProperty](./Hierarchy.xml#L106:~:text=<ComplexType%20Name="-,RecursiveHierarchyType,-")|PropertyPath?|Property of type `Collection(Edm.Boolean)` whose n-th entry is `true` if a node is a start node for the n-th `ancestors` or `descendants` transformation in the order in which they occur in the `$apply` query option
[MatchedDescendantCountsProperty](./Hierarchy.xml#L112:~:text=<ComplexType%20Name="-,RecursiveHierarchyType,-")|PropertyPath?|Property of type `Collection(Edm.Int64)` whose n-th entry is the number of a node's descendants among the start nodes for the n-th `ancestors` or `descendants` transformation in the order in which they occur in the `$apply` query option
[StartNodeCountsSupported](./Hierarchy.xml#L118:~:text=<ComplexType%20Name="-,RecursiveHierarchyType,-")|[Tag](https://github.com/oasis-tcs/odata-vocabularies/blob/main/vocabularies/Org.OData.Core.V1.md#Tag)|Whether the service provides the [`StartNodeCounts`](#StartNodeCounts) instance annotation
