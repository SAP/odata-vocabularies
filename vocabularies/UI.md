...
[DisplayTimeZone](UI.xml#L1288) *([Experimental](Common.md#Experimental))*|String?|<a name="DisplayTimeZone"></a>Display the annotated timestamp, time, or date field in this time zone<p>The display time zone is represented as an offset ±hh:mm and can be a path expression. If a path expression is used to reference a field containing the numeric offset, this offset field can in turn be annotated with [Common.Text](Common.md#Text) pointing to a third field containing the symbolic name or textual description of the display time zone.</p>
